...
export default async function LernenPage() {
  ...
  return (
    <div>
      ...
      {/* Ressourcen-Sektion */}
      <div>
        <h2 className="text-sm font-semibold mb-3" style={{ color: 'var(--muted)' }} dir={isRtl ? 'rtl' : 'ltr'}>
          {SECTION_LABEL[lang]}
        </h2>
        <div className="flex flex-col gap-2">
          {RESOURCES.map(resource => (
            <a
              ...
              className="card flex items-center gap-3 transition-all hover:shadow-sm"
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium truncate" style={{ color: 'var(--foreground)' }}>
                  {resource.labels[lang] ?? resource.de}
                </p>
